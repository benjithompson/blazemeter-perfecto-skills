...
| `bzm-influx-export` | Exports run history — per-run KPI summaries plus minute-by-minute intra-run curves — from an account/workspace/project into **InfluxDB 2.x**: one-shot backfill (default last 7 days) or incremental watermark-based sync, with a Docker quickstart, starter Flux queries (ids as tags, display names via `last()`), and a cron one-liner for user-owned continuous ingestion. |
