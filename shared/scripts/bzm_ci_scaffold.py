...
This module holds the **deterministic** half of the `blazemeter-ci-setup` skill so it
...
def _runner_script(gate: str) -> list[str]:
    ...
    if gate == "pass-fail":
        ...
        gate_lines = [
            ...
            '          # blazemeter-baseline skill and commit it.',
            '          baseline_file = ".blazemeter/baseline.json"',
            '          if not os.path.exists(baseline_file):',
            '              sys.exit("Missing %s — create it with the blazemeter-baseline skill and commit it" % baseline_file)',
...
def build_workflow(
    ...
    *,
    ...
    workflow = "\n".join(
        [
            "# Generated by the blazemeter-ci-setup skill.",
