...
the `/` menu as `/perforce:<name>` (on the CLI, VS Code, and the desktop app alike), and
if a command and a skill share a name the **skill takes precedence**. So a per-skill wrapper here
would be redundant (and ignored), and the `perforce:` namespace applies regardless — a
