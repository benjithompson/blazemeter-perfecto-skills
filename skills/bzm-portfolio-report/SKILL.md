...
Produce the **Portfolio Report**: a single branded, self-contained HTML scorecard that rolls up **every test in a scope** (a workspace or project) over a window (default a quarter) — each test's health, SLA-compliance %, trend, and whether it regressed against **its own baseline**, plus a ranked cross-test incident list. It is the account **activity/usage** view — the quarterly, stakeholder-facing scorecard of how the whole suite is doing over a long window. The **daily ops rollup** — execution results rolled up per workspace/project over a short window, with its own executive HTML — lives in `bzm-daily-digest`; reach for this skill when you want the quarterly/stakeholder view. Where `bzm-test-analysis` trends **one** test across its runs, this skill is its **portfolio sibling** — the same template, the same brand, one row per test instead of one row per run.
...
The Portfolio Report is the **same shipped HTML template** as the single-test Report in `bzm-test-analysis`, rendering itself from the data model in the browser — there is **no Python step and no local interpreter**. New report types plug in at the **data-model seam**, not by forking the renderer: the model's `kind` selects the portfolio section group (scope context, executive summary, portfolio charts, the per-test scorecard, ranked incidents). Produce the file with three deterministic actions:

1. **Read the template** at `${CLAUDE_PLUGIN_ROOT}/shared/assets/report-template.html`. It bakes in the CSS, the approximated-BlazeMeter brand vars, and the vendored client-side JS that dispatches on the model's `kind` and builds the portfolio sections.
...
- **Companion to the digest.** `bzm-daily-digest` is the **daily execution-results rollup** — per workspace/project, markdown-first with its own executive HTML; this skill is the **activity/usage** view, the quarterly stakeholder scorecard. Use the digest for a standup; use this for the quarter.
