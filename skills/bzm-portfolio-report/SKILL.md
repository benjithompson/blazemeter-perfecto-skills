...
Produce the **Portfolio Report**: a single branded, self-contained HTML scorecard that rolls up **every test in a scope** (a workspace or project) over a window (default a quarter) — each test's health, SLA-compliance %, trend, and whether it regressed against **its own baseline**, plus a ranked cross-test incident list. It is the account **activity/usage** view — the quarterly, stakeholder-facing scorecard of how the whole suite is doing over a long window. The **daily ops rollup** — execution results rolled up per workspace/project over a short window, with its own executive HTML — lives in `bzm-daily-digest`; reach for this skill when you want the quarterly/stakeholder view. Where `bzm-report` trends **one** test across its runs, this skill is its **portfolio sibling** — the same template, the same brand, one row per test instead of one row per run.
