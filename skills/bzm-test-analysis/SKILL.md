...
**Intra-run timeseries (optional, Trend and both compare modes):** when the user asks how runs *unfolded* — ramp-up behavior, mid-run spikes or error bursts, saturation, "does p95 degrade over the hold?", curve/shape comparisons — add `--timeseries` to the engine invocation. Each selected run then carries a compact within-run block: a minute-bucketed KPI curve plus a deterministic shape summary. Leave it off otherwise; it adds two API fetches per selected run.
...
- **`--timeseries`** — add when the user asked about within-run behavior (see above). Curves are pulled for the **newest 5 KPI runs only** (the candidate and an in-window baseline are always included) so cost and JSON size stay bounded; raise with `--curve-runs N` when the user explicitly wants shapes across more runs. When enabled, stdout gains a sixth line reporting how many curve pulls succeeded.
...
- Stdout is a **five-line summary** (statuses, KPI availability, adverse moves, fetch coverage, output path) — show it as progress. The full result is the JSON at `--out` (a session scratch path, never the user's repo). `--no-anomalies` skips the anomaly-status fetches if the user doesn't want them. Add `--timeseries` when the user wants the two runs' within-run curves compared (ramp, spikes, saturation) — each side then carries a `timeseries` block.
...
With `--timeseries`, selected runs additionally carry a `timeseries` block (absent = not selected for curves; `null` + a `timeseries_unavailable` note = the pull failed or the run has no aggregate series): `interval_s` (bucket width, 60 s), `start_ts`, `buckets`, `curve` (parallel arrays over downsampled buckets — `offset_s` from run start, `users`, `hits_per_s`, `error_rate_pct`, `avg_ms`, `p95_ms`; `null` entries mean no data in that span, never zero), and `shape` — deterministic within-run features: `peak_users` and `time_to_peak_users_s` (ramp end = users first within 5% of peak); `phases` (`ramp` / `steady_early` / `steady_late`, each with hits/s, error rate, hits-weighted avg and worst-bucket p95); `p95_slope_ms_per_min` (least-squares p95 drift over the hold — sustained positive slope = degradation under steady load); `rt_spikes` (buckets ≥ 2× the run's median avg RT); `error_bursts` (buckets ≥ 5% errors and ≥ 2× the run's overall rate); `saturation` (`throughput_peak_offset_s`, and `throughput_peaked_before_users` — throughput topping out while users still climbed, the classic saturation knee).
...
7. **Intra-run shape (only with `--timeseries`)** — compare the `shape` blocks *across* runs; a one-run quirk is noise, the same feature recurring is structural. Look for: ramp-up degradation (the `ramp` phase's p95 worsening run over run, or spikes clustering at similar `offset_s` early in each run — "p95 blows up at minute 3 in the last five runs" is exactly this); hold degradation (`p95_slope_ms_per_min` consistently positive — the run gets slower the longer it holds, a leak/queue-growth smell); recurring mid-run events (`rt_spikes`/`error_bursts` at similar offsets across runs point at something periodic — cache expiry, batch jobs, GC); and saturation (`throughput_peaked_before_users` with `steady` error rates above `ramp` ones — the test pushes past capacity; the knee's offset and user count say roughly where capacity runs out). Anchor every claim to run ids and offsets, and convert offsets to minutes for the reader.
...
With `--timeseries`, each side carries the same `timeseries` block as Trend mode (curve arrays + `shape`). Use it to say *where inside the run* the regression lives — same aggregate p95 but a new mid-run spike, a positive `p95_slope_ms_per_min` the baseline didn't have, or a saturation knee arriving at fewer users are sharper findings than the run-level delta alone. Shape differences refine the verdict's *reasons*; the verdict itself still comes from `verdict_inputs`.
...
### Intra-run shape                                 ← only when --timeseries was used
- <recurring within-run pattern across runs, with run ids and minute offsets>
- <ramp / hold-degradation / saturation finding, e.g. "p95 climbs ~4 ms/min over the
   hold in each of the last 3 runs" or "throughput tops out at ~14 VU (min 2) while
   users ramp to 20 — saturation knee">
- Curves pulled for runs <ids>; unavailable for <ids>  ← only when something is missing
...
- **`timeseries_unavailable` is not a finding either.** The within-run pull failed or the run exposes no aggregate series — insufficient data. And a run with *no* `timeseries` key wasn't selected for curves at all (only the newest 5 KPI runs, candidate, and in-window baseline are; `--curve-runs` widens it) — don't read absence as a problem with the run.
- **Intra-run curves are minute-grained and downsampled.** Buckets are 60 s (sub-minute blips are smoothed — don't claim "no spikes" at second granularity), long runs are merged to ≤ 60 curve points, and a merged point's p95 is its span's **worst** bucket (conservative, so spikes survive merging; don't read a merged p95 as an average). The `shape` features are computed from the full minute series *before* merging — trust them over eyeballing the merged curve, and don't recompute slopes or spike thresholds yourself.
