...
- **`compare-baseline` (compare vs the committed baseline)** — the job compares this run against the baseline recorded in `.blazemeter/baseline.json` and fails on a regression. Pick this to catch *drift* even when the test has no hard criteria. This requires a committed baseline file (Step 4).
...
3. Paste the contents of the BlazeMeter API-key JSON (`{"id": "...", "secret": "..."}`) as the value. This is the same key the BlazeMeter MCP uses. **Never** commit this value or paste it into the workflow file — the workflow reads it by reference at run time.

If the project touches GitHub through an MCP-backed flow (e.g. opening the PR that adds this workflow, or posting status), use the **GitHub MCP** first — `gh`/REST is only a fallback for a genuine gap. Setting a repository *secret* is done by the user in the GitHub UI (or `gh secret set`), not by this skill: the skill never handles the token.

**Only for `compare-baseline` — commit the baseline file:**
1. Create / update `.blazemeter/baseline.json` (a `test_id → execution_id` map) with the **`bzm-set-baseline`** skill ("promote this run to the baseline"). It writes the file and shows a diff.
...
- **Secrets-only, always.** The generated YAML reads the credential **only** from `${{ secrets.BLAZEMETER_API_KEY }}` and contains no literal key, no key-file path with a value, and no `echo`/`cat` of the secret. Never paste a token into the workflow or into chat. The secret is provisioned once by the user in the GitHub UI; the workflow consumes it by reference. (The scaffold script enforces this; its tests assert no literal credential appears.)
...
- **CI uses REST v4, not the MCP — deliberately.** The BlazeMeter MCP is an interactive server, not a headless CI runner, so the generated job drives the test via the documented REST API v4 (start → poll the master → read the verdict/KPIs). That's the one justified exception to MCP-first here; the skill's *own* context resolution still uses the MCP.
